...
/**
 * 
 * @description 接收视频会话消息
 * @author beyond.zhang   
 */
public class AcceptVideoMessage implements Message {

	public static final short CODE = 211;
	private final static Logger logger = LoggerFactory.getLogger(AcceptVideoMessage.class);
	...
	public void onReceived() {	
		logger.debug("收到接受音频聊天消息user {},inviterId {}",userId,inviterId);
		...
		if (fromEndpoints == null ) {
			...
			logger.info("移除用户{}在TIMEOUT_MANAGER的记录",inviterId);
			try {
				Context.TIMEOUT_MANAGER.deregister(Context.ID_TO_SLOT.get(inviterId));
			} catch (Exception e) {
				logger.error("收到接受音频聊天消息发生异常：",e);
			}
			if (toEndpoints != null) {
	...
	public void fromBinary(ByteBuf buffer) {
		...
		byte inviterNameLen = (byte) buffer.readUnsignedByte();
	...
	public String toJson() {
		...
		try {
			...
			logger.error("接收视频会话消息code(211)异常："+e);
		}
		return str;
	}
