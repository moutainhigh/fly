...
public class DontHaveCameraMessage implements Message {

	public static final short CODE = 242;
	private final static Logger logger = LoggerFactory.getLogger(DontHaveCameraMessage.class);
	...
	public short getCode() {
		return DontHaveCameraMessage.CODE;
	}
	...
	public void onReceived() {
		logger.debug("收到没有摄像头消息senderId {},receiverId {}",senderId,receiverId);
		List<Endpoint> fromEndpoints = Context.getEndpoints(senderId);
		List<Endpoint> toEndpoints = Context.getEndpoints(receiverId);
		Context.deregisterVA(senderId, receiverId);
        Context.deregisterPAV(senderId,receiverId);
        Context.deregisterPAV(receiverId,senderId);
	...
	public String toJson() {
		...
		try {
			...
			logger.error("DontHaveCameraMessage 消息异常："+e);
		}
		return str;
	}
