...
public class ChatMessage implements Message {
	...
	private final static Logger logger = LoggerFactory.getLogger(ChatMessage.class);
	...
	public void onReceived() {
		logger.debug("收到聊天邀请消息senderId {},receiverId {}",senderId,receiverId);
		...
		MemCachedUtil.setInChat(senderId,receiverId,1L);//设置成在聊天室状态
		if (fromEndpoints == null ) {
			Utils.close(endpoint.getContext());
		} else {
			synchronized(fromEndpoints){
			UpdateNumMessage upMessage = MessageFactory.createUpdateNumMessage();
			upMessage.setEndpoint(endpoint);
			for(Endpoint fromEndpoint : fromEndpoints){
				if (fromEndpoint instanceof SocketEndpoint) {
					fromEndpoint.getContext().channel().writeAndFlush(upMessage.toBinary());
				} else {
					fromEndpoint.getContext().channel()
							.writeAndFlush(new TextWebSocketFrame(upMessage.toJson()));
				}
                if(fromEndpoint != endpoint){
                    if (fromEndpoint instanceof SocketEndpoint) {
                        fromEndpoint.getContext().channel().writeAndFlush(this.toBinary());
                    } else {
                        fromEndpoint.getContext().channel().writeAndFlush(new TextWebSocketFrame(this.toJson()));
                    }
                }
			}
			}
		...
		Context.registerWebRTCEndpointPeer(endpoint, receiverId);
		}
	}
