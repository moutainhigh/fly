 ...
 * @author beyond.zhang
 */
public class LackTimeMessage implements Message {

	public static final short CODE = 151;
	private final static Logger logger = LoggerFactory.getLogger(LackTimeMessage.class);
	...
	public ByteBuf toBinary() {
		...
		buffer.writeLong(consultId);
		buffer.writeByte(left);
		return buffer;
	}
	...
	public void fromBinary(ByteBuf buffer) {
		...
		consultId = buffer.readLong();
		left = buffer.readUnsignedByte();
	}
	...
	public void fromMap(Map<String, JsonNode> map) {
		...
		consultId = map.get("consultId").asLong();
		left = Byte.parseByte(map.get("left")+"");
		this.length = 2 + 8 + 8 + 6 + 8 + 2 * 1+1
