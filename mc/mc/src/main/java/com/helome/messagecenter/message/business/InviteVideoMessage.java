 ...
 * @author beyond.zhang
 */
public class InviteVideoMessage implements Message, Timeout {

	public static final short CODE = 210;
	private final static Logger logger = LoggerFactory
			.getLogger(InviteVideoMessage.class);
	...
	public void onReceived() {
        if(Context.existInVA(userId, inviteeId)){
			...
			if (fromEndpoints != null) {
				synchronized (fromEndpoints) {
					for (Endpoint e : fromEndpoints) {
						Message m = MessageFactory.createMeNotAvailable(userId,
								userName, inviteeId, inviteeName);
						m.setEndpoint(e);
						if (e instanceof SocketEndpoint) {
							e.getContext().channel()
									.writeAndFlush(m.toBinary());
						} else {
							e.getContext()
									.channel()
									.writeAndFlush(
			...
			if (fromEndpoints != null) {
				synchronized (fromEndpoints) {
					for (Endpoint e : fromEndpoints) {
						Message m = MessageFactory.createYouNotAvailable(
								userId, userName, inviteeId, inviteeName);
						m.setEndpoint(e);
						if (e instanceof SocketEndpoint) {
							e.getContext().channel()
									.writeAndFlush(m.toBinary());
			...
			if (fromEndpoints == null) {
				...
				if (Context.registerVA(userId, inviteeId)) {
					...
					Context.ID_TO_SLOT.put(userId,
							Context.TIMEOUT_MANAGER.register(this));
					if (toEndpoints != null) {
						synchronized (toEndpoints) {
							for (Endpoint toEndpoint : toEndpoints) {
								if (toEndpoint instanceof SocketEndpoint) {
					...
					logger.info("邀请者{}与被邀请者{}注册在视频中失败",userId,inviteeId);
	...
	public void onTimeout() {
		...
		if (fromEndpoints != null) {
			for (Endpoint e : fromEndpoints) {
				Message m = MessageFactory.createVideoTimeoutMessage(userId,
						userName, inviteeId, inviteeName);
				m.setEndpoint(e);
				if (e instanceof SocketEndpoint) {
					e.getContext().channel().writeAndFlush(m.toBinary());
					...
					e.getContext().channel()
							.writeAndFlush(new TextWebSocketFrame(m.toJson()));
				}
			}
		}
	}
